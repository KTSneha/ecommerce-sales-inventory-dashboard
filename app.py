...
with tab1:
    ...
    revenue_trend = (
        filtered_df
        .groupby("Date")["Revenue_After_Discount"]
        .sum()
        .reset_index()
        .sort_values("Date")
    )

    revenue_trend["7-Day Avg"] = (
        revenue_trend["Revenue_After_Discount"]
    ...
    fig_trend = px.line(
        revenue_trend,
        x="Date",
        y=["Revenue_After_Discount", "7-Day Avg"],
    ...
    fig_trend.update_traces(
        opacity=0.3,
        selector=dict(name="Revenue_After_Discount")
    )

    fig_trend.update_traces(
        line=dict(width=3),
        selector=dict(name="7-Day Avg")
    )
    ...
    with col5:
        ...
        revenue_by_category = (
            filtered_df
            .groupby("Category")["Revenue_After_Discount"]
            .sum()
            .reset_index()
            .sort_values(
        ...
        fig_cat.update_yaxes(
            range=[
                revenue_by_category["Revenue_After_Discount"].min() * 0.9,
                revenue_by_category["Revenue_After_Discount"].max() * 1.05
            ]
        )
        ...
        st.caption(
            ...
            f"**{gap_pct:.1f}%** more revenue than the lowest category, "
    ...
    with col6:
        ...
        revenue_by_region = (
            filtered_df
            .groupby("Region")["Revenue_After_Discount"]
            .sum()
            .reset_index()
            .sort_values(
                "Revenue_After_Discount",
        ...
        fig_region.update_yaxes(
            range=[
                revenue_by_region["Revenue_After_Discount"].min() * 0.9,
                revenue_by_region["Revenue_After_Discount"].max() * 1.05
            ]
        )
        ...
        st.caption(
            ...
            f"**{gap_pct_reg:.1f}%** more revenue than the lowest region, "
            f"**{bottom_reg['Region']}**."
        )
    ...
    promo_impact = (
        filtered_df
        .groupby("Holiday/Promotion")["Revenue_After_Discount"]
        .mean()
        .reset_index()
    )
    ...
    no_promo_val = promo_impact.loc[
        promo_impact["Holiday/Promotion"] == "No Promotion",
        "Revenue_After_Discount"
    ].values[0]

    promo_val = promo_impact.loc[
        promo_impact["Holiday/Promotion"] == "Promotion",
        "Revenue_After_Discount"
    ].values[0]
    ...
    fig_promo.update_yaxes(
        range=[
            min(no_promo_val, promo_val) * 0.9,
            max(no_promo_val, promo_val) * 1.1
        ]
    )
    ...
    st.caption(
        f"Promotions show a **{pct_diff:+.1f}%** difference in "
        f"average revenue per transaction — a negligible effect in "
        f"this dataset, suggesting promotions here don't meaningfully "
        f"drive higher order value."
    )
...
with tab3:
    ...
    st.caption(
        ...
        f"(Avg Daily Sales × Lead Time) + Safety Stock Buffer "
